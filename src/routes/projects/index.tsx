...
function ProjectsPage() {
  ...
  const projects = rawProjects.map((p) => ({
    ...
    progress: (p as any).progress || (p.budget && p.spent ? Math.min(Math.round((p.spent / p.budget) * 100), 100) : 0),
    status: p.status || "On Track",
    budget: typeof p.budget === "number" ? `₹${(p.budget / 10_000_000).toFixed(2)} Cr` : p.budget || "₹0.00 Cr",
    spent: typeof p.spent === "number" ? `₹${(p.spent / 10_000_000).toFixed(2)} Cr` : p.spent || "₹0.00 Cr",
    deadline: p.endDate || "Dec 2026",
    image: (p as any).image || "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=800&q=80",
  ...
  const presetImages = [
    { label: "🏢 High-Rise Apartment", url: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=800&q=80" },
    { label: "🏗️ Commercial Structure", url: "https://images.unsplash.com/photo-1503387762-592dec58ef4e?auto=format&fit=crop&w=800&q=80" },
    { label: "🚧 Excavation Site", url: "https://images.unsplash.com/photo-1578328819058-b69f3a3b0f6b?auto=format&fit=crop&w=800&q=80" },
    { label: "🏡 Luxury Residential", url: "https://images.unsplash.com/photo-1613490493576-7fde63acd811?auto=format&fit=crop&w=800&q=80" },
    { label: "🛠️ Interior Fit-out", url: "https://images.unsplash.com/photo-1513694203232-719a280e022f?auto=format&fit=crop&w=800&q=80" },
  ];
      ...
      {isAddProjectOpen ? (
        <div className="space-y-6 animate-fade-up">
          ...
          <div className="border-b border-border pb-4">
            <h1 className="text-3xl font-display font-semibold tracking-tight">Create New Project</h1>
            <p className="text-xs text-muted-foreground mt-1">
              Initialize a new construction site, set targets, allocate budgets, and assign preset gallery feeds.
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* Form Section */}
            <form onSubmit={handleAddProject} className="lg:col-span-8 bg-[color:var(--surface)] border border-border rounded-xl shadow-sm p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="font-semibold text-muted-foreground">Location (City, State) *</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="font-semibold text-muted-foreground">Total Budget (₹ Cr) *</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="font-semibold text-muted-foreground">Project Site Type / Preset Photo</label>
                  <select
                    value={newProjImage}
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="font-semibold text-muted-foreground">Or Paste Custom Photo URL (Optional)</label>
                  <input
                    type="url"
                ...
                <div className="space-y-1.5">
                  <label className="font-semibold text-muted-foreground">Progress Percentage (0-100)</label>
                  <input
                    type="number"
                    min="0"
                    max="100"
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="font-semibold text-muted-foreground">Default Site Supervisor</label>
                  <select
            ...
            <div className="lg:col-span-4 space-y-4 shrink-0">
              <h3 className="text-xs font-mono uppercase text-muted-foreground tracking-wider font-semibold">Live Card Preview</h3>
              <article className="bg-[color:var(--surface)] rounded-xl border border-border overflow-hidden flex flex-col shadow-sm">
                <div className="relative h-44 w-full bg-secondary overflow-hidden">
                  <img 
                    src={customImageUrl || newProjImage || "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?auto=format&fit=crop&w=800&q=80"} 
                ...
                <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                  <div>
                    <div className="flex justify-between items-baseline mb-2">
                      <span className="text-xs text-muted-foreground">{newProjPhase || "Structure"}</span>
                  ...
                  {newProjSupervisor && newProjSupervisor !== "None" && (
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground bg-secondary/20 rounded-lg p-2.5 my-1.5 border border-border/40 shrink-0 animate-fade-in">
                      ...
                      <span className="truncate font-medium">Supervisor: <strong className="text-foreground">{newProjSupervisor}</strong></span>
                    </div>
                  )}
                  ...
                  <dl className="grid grid-cols-3 gap-3 pt-3 border-t border-border text-xs">
                    <div>
                      ...
                      <dd className="font-mono font-medium mt-0.5">₹{newProjBudget || "0.00"} Cr</dd>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <dd className="font-mono font-medium mt-0.5">{newProjDeadline || "Dec 2026"}</dd>
                    </div>
                  </dl>
                  <div className="grid grid-cols-3 gap-2 pt-3 border-t border-border">
                    <button className="py-1.5 text-xs font-medium bg-secondary text-secondary-foreground rounded hover:bg-secondary/80 transition-colors">View</button>
                    <button className="py-1.5 text-xs font-medium bg-secondary text-secondary-foreground rounded hover:bg-secondary/80 transition-colors">Edit</button>
                    <button className="py-1.5 text-xs font-medium bg-secondary text-secondary-foreground rounded hover:bg-secondary/80 transition-colors">Analytics</button>
                  </div>
                </div>
              </article>
            </div>
            ...
            {projects.map((p, i) => (
              ...
              >
                ...
                <div className="relative h-44 w-full bg-secondary overflow-hidden shrink-0">
                  <img 
                    src={p.image || "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?auto=format&fit=crop&w=800&q=80"} 
                    alt={p.name} 
                ...
                <div className="p-5 flex-1 flex flex-col justify-between gap-4">
                  ...
                  {p.supervisor && (
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground bg-secondary/20 rounded-lg p-2.5 my-1.5 border border-border/40 shrink-0">
                      <User className="size-3.5 text-primary shrink-0" />
                      <span className="truncate font-medium">Supervisor: <strong className="text-foreground">{p.supervisor}</strong></span>
                    </div>
                  )}
                  ...
                  <dl className="grid grid-cols-2 gap-3 pt-3 border-t border-border text-xs">
                    ...
                    <div>
                      <dt className="text-muted-foreground mb-1">Open Issues</dt>
                      <dd className={`font-mono font-medium ${(p.openIssues || 0) > 0 ? "text-destructive" : ""}`}>{p.openIssues || 0} pending</dd>
                    </div>
                  </dl>
                  <dl className="grid grid-cols-3 gap-3 pt-3 border-t border-border text-xs">
                    <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-3 border-t border-border">
                    ...
                    <button 
                      onClick={(e) => { e.stopPropagation(); navigate({ to: `/projects/${p.id}/reports` }); }}
                      className="py-1.5 text-xs font-medium bg-secondary text-secondary-foreground rounded hover:bg-secondary/80 transition-colors"
                    >
                      Analytics
                    </button>
