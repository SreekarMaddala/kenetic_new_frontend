...
export const Route = createFileRoute("/projects/$projectId/drawings")({ component: Page });
function Page() {
  const { projectId } = Route.useParams();
  const admin = useAuth().user?.role !== "supervisor";
  const endpoint = `/projects/${projectId}/drawings`;
      ...
      actions={[
        {
          ...
          run: async (r) => {
            const result = await api.get<{ url: string }>(
              `${recordUrl(endpoint, r.drawingId)}/download`,
            );
