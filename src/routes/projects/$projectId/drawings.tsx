...
import { Search, Filter, PenTool, LayoutTemplate, Zap, Download, Maximize2, MoreHorizontal } from "lucide-react";
...
function DrawingsPage() {
  ...
  const { data: rawDrawings = [] } = useQuery({ queryKey: ["drawings", projectId], queryFn: () => documentControlApi.listDrawings(projectId) });
  const drawings = rawDrawings.map((drawing) => ({
    id: String(drawing.drawingId ?? ""), title: String(drawing.title ?? drawing.name ?? "Untitled drawing"),
    category: String(drawing.category ?? "Architectural"), rev: String(drawing.revision ?? drawing.rev ?? "—"),
    status: String(drawing.status ?? "Pending Review"), date: String(drawing.createdAt ?? ""), author: String(drawing.author ?? "—"),
    icon: <LayoutTemplate className="size-5 text-primary" />, bg: "bg-primary/10",
  }));
    ...
    <div className="p-8 max-w-7xl mx-auto w-full space-y-6 animate-fade-up">
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${
                categoryFilter === cat ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:bg-secondary/80"
              }`}
            >
              {cat}
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredDrawings.map((dwg) => (
          <div key={dwg.id} className="group rounded-xl border border-border bg-[color:var(--surface)] overflow-hidden hover:border-primary/40 hover:shadow-md transition-all">
            ...
            <div className="h-40 bg-secondary/30 border-b border-border relative flex items-center justify-center p-6">
              <div className="absolute top-3 right-3 flex gap-1">
                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                  dwg.status === "Approved (GFC)" ? "bg-emerald-500/10 text-emerald-600" :
                  dwg.status === "Pending Review" ? "bg-amber-500/10 text-amber-600" :
              ...
              <div className="absolute inset-0 bg-background/80 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                <button className="size-10 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg hover:scale-105 transition-transform" title="View Full Screen">
                  <Maximize2 className="size-4" />
                </button>
                <button className="size-10 rounded-full bg-white text-foreground border shadow-lg flex items-center justify-center hover:scale-105 transition-transform" title="Download PDF">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <h4 className="font-semibold text-sm text-foreground line-clamp-1">{dwg.title}</h4>
                </div>
                <div className={`size-8 rounded flex items-center justify-center shrink-0 ${dwg.bg}`}>
                  {dwg.icon}
                </div>
              </div>
