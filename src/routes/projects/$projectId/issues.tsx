...
function IssuesPage() {
  ...
  const issues = rawIssues.map((issue) => ({
    ...
    title: String(issue.title ?? issue.description ?? "Untitled issue"),
      ...
      <div className="bg-[color:var(--surface)] rounded-xl border border-border shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="divide-y divide-border">
          {filteredIssues.map((iss) => (
            <div
              key={iss.id}
              className="p-5 hover:bg-secondary/20 transition-colors cursor-pointer group flex flex-col sm:flex-row sm:items-center gap-4"
            >
              {/* Icon & Details */}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1.5">
                      <Clock className="size-3" /> Opened {iss.date}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <MessageSquare className="size-3" /> {iss.comments} Comments
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2 shrink-0 sm:w-40 pt-4 sm:pt-0 border-t sm:border-t-0 border-border">
                <div className="text-xs font-medium text-foreground">{iss.assignedTo}</div>
                <span
